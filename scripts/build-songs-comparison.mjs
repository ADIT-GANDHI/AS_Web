 ...
 * Run: node scripts/build-songs-comparison.mjs [baseUrl] [--only listing|filter|detail]
 */
...
const OUT_ROOT = path.join(ROOT, 'Songs_Localhost_Comparison');

const LISTING_DIR = path.join(OUT_ROOT, '1_Songs_Listing');
const FILTER_DIR  = path.join(OUT_ROOT, '2_Filter_Panel');
const DETAIL_DIR  = path.join(OUT_ROOT, '3_Song_Details');
const TARGET_W = 1440;
...
const pyPath = path.join(OUT_ROOT, '_extract_songs_pdf.py');
...
ROOT = Path(${JSON.stringify(OUT_ROOT)})
...
const runListing = !only || only === 'listing';
const runFilter  = !only || only === 'filter';
const runDetail  = !only || only === 'detail';

// ── 2. Browser screenshots via Playwright ────────────────────────────────────
if (runListing || runFilter || runDetail) {
...
if (runListing) {
  ...
  // Full page (+ README alias)
  await page.screenshot({ path: path.join(LISTING_DIR, 'live-listing-full.png'), fullPage: true });
  await page.screenshot({ path: path.join(LISTING_DIR, 'localhost-songs-listing.png'), fullPage: true });
...
if (runFilter) {
  ...
  await page.click('.cl-filter-trigger').catch(() =>
    page.click('button:has-text("Filters")').catch(() => page.click('[class*="filter"]').catch(() => {}))
  );
...
// ── 3. Overlay red guides on live captures + stitch ───────────────────────────
const pyStitch = path.join(OUT_ROOT, '_stitch_songs.py');
...
def guide_live(src_path, out_path, v_lines, h_lines, artboard_w):
    img = Image.open(src_path).convert("RGB")
    guided = overlay_guides(img, v_lines, h_lines, artboard_w)
    guided.save(str(out_path), quality=93)
    print(f"  Guided: {out_path.name}")
...
print("\\nOverlaying guides on live captures...")
if ONLY in (None, "listing"):
    live = LISTING / "live-listing-full.png"
    if live.exists():
        guide_live(live, LISTING / "live-listing-guided.png", SONGS_V, SONGS_H, ARTBOARD_SONGS)

if ONLY in (None, "filter"):
    live = FILTER / "live-filter-with-selections.png"
    if live.exists():
        guide_live(live, FILTER / "live-filter-guided.png", FILTER_V, FILTER_H, ARTBOARD_SONGS)

if ONLY in (None, "detail"):
    live = DETAIL / "live-detail-full.png"
    if live.exists():
        guide_live(live, DETAIL / "live-detail-guided.png", DETAIL_V, DETAIL_H, ARTBOARD_DETAIL)
...
if ONLY in (None, "listing"):
    stitch(
        ...
        LISTING / "live-listing-guided.png",
        "PDF — Page 1 Songs listing  ·  2.SongsAll_Detailpg_01.05.2025.pdf",
        f"Browser — {BASE_URL}/songs  ·  {TARGET_W}px",
...
if ONLY in (None, "filter"):
    stitch(
        ...
        FILTER / "live-filter-guided.png",
        "PDF — Page 2 Filter panel  ·  2.SongsAll_Detailpg_01.05.2025.pdf",
        f"Browser — {BASE_URL}/songs (Filters open)  ·  {TARGET_W}px",
...
if ONLY in (None, "detail"):
    stitch(
        ...
        DETAIL / "live-detail-guided.png",
        "PDF — Page 3 Song detail  ·  2.SongsAll_Detailpg_01.05.2025.pdf",
        f"Browser — Song detail  ·  {TARGET_W}px",
    ...
    if crop_pdf.exists() and crop_live.exists():
        guide_live(crop_live, DETAIL / "live-detail-panel-guided.png", DETAIL_V, [h for h in DETAIL_H if h < 900 / (TARGET_W / ARTBOARD_DETAIL)], ARTBOARD_DETAIL)
        stitch(
            crop_pdf,
            DETAIL / "live-detail-panel-guided.png",
